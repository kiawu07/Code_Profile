...
class Invest():
    ...
    def info_file(self,account,client):
        ...
        with open(str(info_path),'w') as output_file:
            ...
            output_file.write(f"BALANCE: ${account.get_balance():,.2f}\n")
            output_file.write("STATEMENTS:\n")
            output_file.write(f"{account.get_statement()}\n\n")
            output_file.write("=======Investment Account Info========\n")
            output_file.write(f"Available to trade: ${self.get_available_to_trade():,.2f}\n")
            output_file.write(f"Investment Net Worth: ${self.networth():,.2f}\n")
